...
void TCPSender::fill_window() {
    ...
    if(!_segments_wait.empty() && _segments_wait.front().header().syn) return;
    ...
    while(remaining_size > 0 && !_stream.buffer_empty()){
        ...
        if(_stream.eof()
            && tcpSegment.length_in_sequence_space() < remaining_size){
            tcpSegment.header().fin = true;
            _fin_send = true;
        }
        _send_segment(tcpSegment);
...
void TCPSender::ack_received(const WrappingInt32 ackno, const uint16_t window_size) {
    ...
    while(!_segments_wait.empty()){
        ...
        if(unwrap(front.header().seqno, _isn, _stream.bytes_read()) + front.length_in_sequence_space() <=
            ab_sn){
            //更新 _bytes_in_flight
